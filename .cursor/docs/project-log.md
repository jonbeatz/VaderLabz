...
| 2026-07-03 | Skill library v1.14.0 + sync workflow | Added 5 interactive-web skills to shared library (Scroll-Motion, Scroll-Video-Sequence, Component-Registries, View-Transitions, Motion-Accessibility) + vendored frontend-design. New `sync-skills.ps1` (`npm run sync:skills`) refreshes `.cursor/skills` from the library; auto-runs at Start Project. New `sync-docs.ps1` (`npm run sync:docs` preview / `-Write` apply). Committed + pushed. |
